        ...
        <div class="mx-4">
                ...
                <div class="flex flex-wrap ">
                    ...
                    <div class="w-full mt-4 flex flex-wrap items-center justify-between">
                        ...
                        <div class="text-right ml-4">
                            <button @click="createOpen = true" class="bg-blue-500 dark:bg-blue-600 font-bold tracking-wide text-white rounded px-6 py-2 shadow">
                                Create {{singular}}
                            </button>
                        </div>
                    </div>

                    <div class="w-full bg-white dark:bg-gray-500 shadow rounded-lg mt-4 flex flex-wrap items-center justify-between">
                        <div class="bg-gray-600 relative border-b border-gray-300 w-full p-4 flex flex-wrap justify-between items-center rounded-t-lg">
                            <div>
                                <input @change="selectAll" type="checkbox">
                            </div>
                            <button @click="filtersOpen= !filtersOpen" class="focus:outline-none flex flex-wrap items-center p-2 rounded-lg" :class="{'bg-gray-300 dark:bg-gray-700': filtersOpen, 'bg-gray-100 dark:bg-gray-800': !filtersOpen}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg>
                                <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </button>

                            <div v-if="filtersOpen" class="absolute bg-white dark:bg-gray-700 shadow-lg top-0 right-0 mt-14 mr-4 border border-gray-200 dark:border-gray-500 rounded-lg" style="width: 250px;">
                                <div class="bg-gray-100 dark:bg-gray-800 uppercase py-2 px-2 font-bold text-gray-500 dark:text-gray-400 text-sm rounded-t-lg">
                                    filters
                                </div>
                                <div class="flex flex-wrap items-center p-2">
                                    <select v-model="itemsPerPage" class="border border-gray-300 rounded-lg w-full p-1 dark:border-gray-600 dark:bg-gray-600">
            ...
            <div v-if="createOpen" class="fixed z-0 inset-0 flex items-center outline-none w-screen h-screen overflow-y-scroll">
                <div class="relative z-10 w-full md:w-1/2 mx-auto max-h-screen overflow-y-auto">
                    <div class="w-full rounded p-4 bg-white dark:bg-gray-600 shadow-lg text-left">
                        ...
                        <div class="flex flex-col border-t border-gray-200 mt-2 pt-4">
                            <slot name="form"></slot>
                            <div class="mt-4 flex justify-end">
                                <button @click.prevent="() => {
                                    $emit('save', form);
                                    createOpen = false;
                                    }" class="border border-blue-500 dark:border-blue-600 darK:border-blue-600 dark:bg-blue-600 dark:text-white py-2 px-4 rounded text-blue-500 hover:bg-blue-500 hover:text-white">
                                    Save
                                </button>
                            </div>
                        </div>
