<template>
    <div class="h-full flex flex-col">
        ...
        <div class="grid flex-grow" :class="'grid-cols-' + headers.length">
            <div v-for="blankday in blankDaysAtTheStartOfTheMonth" class="bg-gray-300" v-if="$store.getters.calendarOptions.type === 'month'">
                <div class="h-full border text-left p-1 text-sm">{{ blankday }}</div>
            </div>
            <div v-for="(date, dateIndex) in daysInTheMonth"  :key="dateIndex" class="h-full" >
                <div class="h-full cursor-pointer border-t border-r flex flex-col">
                    <div class="h-full flex flex-col flex-grow" >
                        <div class="flex items-center w-6 h-6 m-1 rounded-full justify-center" :class="{'bg-blue-500 text-white': isToday(date) }">{{ date }}</div>
                        ...
                        <div v-else class="flex flex-col flex-grow gap-1" v-if="$store.getters.events[eventIndex(date)]">
                            <!-- Timeslots -->
                            <div class="px-2 border flex items-center gap-1" v-for="event in $store.getters.events[eventIndex(date)]" :key="'event'+event.id" :style="'border-color:' + event.calendarEvent.color">
                                <div class="flex text-xs">
                                    <span v-if="event.instanceDate.format('m') != '0'">{{ event.instanceDate.format('H:mma') }}</span>
                                    <span v-else>{{ event.instanceDate.format('Ha') }}</span>
                                </div>
            ...
            <div class="flex-grow border bg-gray-300" v-for="index in daysAfterTheMonth" v-if="$store.getters.calendarOptions.type === 'month'">{{ index }}</div>
        </div>
    </div>
</template>
